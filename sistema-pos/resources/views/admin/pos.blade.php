<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-900 leading-tight">POS Moda - Facturación</h2>
    </x-slot>
            ...
            <div class="pos-shell">
                ...
                <section class="pos-center">
                    ...
                    <div class="pos-search-grid">
                        <div>
                            ...
                            <input id="product-search" list="products-list" type="text" placeholder="Buscar por nombre o SKU..."
                                class="pos-input">
                            <datalist id="products-list">
                                @foreach ($products as $product)
                                    <option value="{{ $product['nombre'] }} ({{ $product['sku'] }})"></option>
                                @endforeach
                            </datalist>
                        </div>
                        <div>
                ...
                <section class="pos-right">
                    <div class="pos-panel-head right">
                        ...
                        <span class="text-xs text-gray-500">Caja principal</span>
                    </div>
                    ...
                    <div class="pos-form-grid">
                        ...
                        <div>
                            <label class="pos-label">Numeracion</label>
                            <select class="pos-input">
                                <option>Principal</option>
                                <option>Punto fisico</option>
                    ...
                    <div class="mt-3">
                        ...
                        <input id="customer-search" list="customers-list" type="text" placeholder="Buscar cliente por nombre o documento..."
                            class="pos-input">
                        <datalist id="customers-list">
                            @foreach ($customers as $customer)
                                <option value="{{ $customer['nombre'] }}{{ $customer['numero_documento'] ? ' (' . $customer['numero_documento'] . ')' : '' }}"></option>
                            @endforeach
                        </datalist>
                    </div>
                    ...
                    <div class="pos-cart">
                        <table>
                            ...
                            <tbody id="cart-body">
                                <tr id="cart-empty">
                                    <td colspan="4" class="pos-cart-empty">Aqui veras los productos elegidos para la venta.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <button type="button" class="pos-btn-sell" id="btn-vender">Vender</button>
                </section>
            </div>
        </div>
    </div>
    ...
    <div id="pay-modal" class="pay-modal-overlay" style="display:none">
        <div class="pay-modal-box">
            ...
            <div id="pay-step-1">
                ...
                <button type="button" id="pay-submit-btn" class="pos-btn-sell" style="margin-top:8px">
                    Confirmar pago
                </button>
    ...
    <style>
        ...
        .pos-nav { margin-top: 16px; display: flex; flex-direction: column; gap: 6px; }
        .pos-nav-item { padding: 9px 10px; border-radius: 10px; font-size: 13px; color: #374151; transition: .2s; text-decoration: none; }
        .pos-nav-item:hover { background: #eceef1; }
        .pos-nav-item.is-active { background: #111827; color: #fff; font-weight: 600; }
        .pos-sidebar-footer { font-size: 11px; display: grid; gap: 3px; }
        ...
        .pos-panel-head.right h3 { font-size: 20px; }
        ...
        .pos-add-wrap { display: flex; align-items: end; }
        .pos-label { display: block; margin-bottom: 4px; font-size: 12px; color: #6b7280; }
        .pos-input { width: 100%; border: 1px solid #d1d5db; border-radius: 10px; font-size: 13px; padding: 9px 10px; background: #fff; }
        .pos-input:focus { outline: none; border-color: #111827; box-shadow: 0 0 0 2px rgba(17,24,39,.1); }
        .pos-btn-main { width: 100%; border: 0; border-radius: 10px; background: #111827; color: #fff; padding: 10px 12px; font-size: 13px; font-weight: 600; }
        .pos-card-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 10px; }
        .pos-product-card { border: 1px solid #e5e7eb; border-radius: 12px; background: #fff; padding: 11px; text-align: left; cursor: pointer; transition: .2s; }
        ...
        .pos-btn-sell { margin-top: 12px; border: 0; border-radius: 10px; background: #111827; color: #fff; padding: 11px 12px; font-size: 14px; font-weight: 700; }
        @media (max-width: 1280px) {
            .pos-shell { grid-template-columns: 200px 1fr; }
            .pos-right { grid-column: 1 / -1; border-top: 1px solid #e5e7eb; }
            .pos-center { border-right: 0; }
        }
        @media (max-width: 900px) {
            .pos-shell { grid-template-columns: 1fr; }
            .pos-sidebar { border-right: 0; border-bottom: 1px solid #e5e7eb; }
        ...
        .pos-receipt-ref { font-size:11px;color:#9ca3af;text-align:right }
        ...
        @media print {
            ...
            #pos-receipt { position:fixed;inset:0;overflow:auto }
    ...
    (() => {
        ...
        const matchProduct = v => products.find(p => v.includes(p.sku) || v.toLowerCase().includes(p.nombre.toLowerCase()));
        ...
        const render = () => {
            ...
            cart.forEach((item, i) => {
                ...
                tr.dataset.row = '1';
                tr.innerHTML = `
                    <td><p style="font-weight:600">${item.nombre}</p><p style="font-size:11px;color:#6b7280">${item.sku}</p></td>
                    <td>${item.cantidad}</td>
                    <td>${fmt(item.subtotal)}</td>
                    <td style="text-align:right"><button type="button" data-remove="${i}" style="font-size:11px;color:#dc2626">Quitar</button></td>`;
        ...
        const addToCart = (selected) => {
            ...
            cart.push({ id: selected.id, nombre: selected.nombre, sku: selected.sku,
                         precio: Number(selected.precio || 0), cantidad: qty,
                         subtotal: qty * Number(selected.precio || 0) });
            productInput.value = '';
            qtyInput.value = '1';
            render();
        };

        addButton?.addEventListener('click', () => addToCart(matchProduct(productInput.value.trim())));
        productCards.forEach(card => card.addEventListener('click', () =>
            addToCart({ id: Number(card.dataset.id), nombre: card.dataset.name,
                        sku: card.dataset.sku, precio: Number(card.dataset.price || 0) })
        ));
        ...
        const openModal = () => {
            ...
            // Llenar resumen
            let rows = cart.map(i => `<div class="pay-summary-row"><span>${i.nombre} × ${i.cantidad}</span><span>${fmt(i.subtotal)}</span></div>`).join('');
            const total = cart.reduce((s,i) => s + i.subtotal, 0);
            rows += `<div class="pay-summary-total"><span>Total</span><span>${fmt(total)}</span></div>`;
        ...
        document.getElementById('pay-nueva-venta')?.addEventListener('click', () => {
            cart.length = 0; render(); closeModal();
        });
        ...
        submitBtn?.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch(VENDER_URL, {
                    method: 'POST',
        ...
        const buildReceipt = (d) => {
            ...
            <div class="pos-receipt-wrap">
                <div class="pos-receipt-head">
                    <div>
                        <p class="pos-receipt-brand">POSS ATELIER</p>
                        <p style="font-size:11px;color:#9ca3af;margin-top:2px">Moda & Textiles · NIT 900.123.456-7</p>
                    </div>
                    <div class="pos-receipt-ref">
                        <p style="font-size:10px;color:#9ca3af;letter-spacing:.15em">FACTURA POS</p>
                        <p style="font-weight:700;margin-top:2px">${d.reference}</p>
                    </div>
                </div>
